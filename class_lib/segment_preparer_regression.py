...
class SegmentsPreparer():
    ...
    def prepare_graph(self, X, feature_names, clf, label, feature=0, show_class_labels = False):
        graph = tree.export_graphviz(clf, feature_names=feature_names, out_file='tmp.dot', node_ids=True, 
                                     proportion=True, filled=True, rounded=True, impurity=False)
    ...
    def best_worst_feature_values(self, X, data, clf, cat_cols, float_cols, tree_type):
        ...
        best_node = node_data.loc[node_data['value'].idxmax(), 'node']
        worst_node = node_data.loc[node_data['value'].idxmin(), 'node']
