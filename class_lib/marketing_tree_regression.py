...
class DecisionTree(BaseEstimator):
    ...
    def get_feature_name(self, index):
        full_feature = self.features_names[index]
        pass
    ...
    def get_leaves_info(self, raw=False):
        res = {}
        for leaf in self.leaves_nums:
            node_array = self.decision_path(self.X).toarray()
            samples_in_node_mask = (node_array[:,leaf] == 1)
            if raw:
                res[leaf] = pd.DataFrame(data=self.X[samples_in_node_mask], columns=self.features_names)
            else:
                res[leaf] = pd.DataFrame(data=self.X[samples_in_node_mask], columns=self.features_names).describe().transpose()
        return res
    ...
    def choose_best_split(self, X_subset, y_subset, parents_path, depth, cur_c=0, search_board=True):
        """
        ...
        if search_board and self.c != 0:
            min_c, max_c = self.get_init_c(X_subset, y_subset, parents_path, depth, 0.35)
            c = self.get_lower_bound_on_c(X_subset, y_subset, parents_path, depth, 0.35, min_c, max_c)
        else:
            c = cur_c
        ...
        for feature_idx in range(n_features):
            ...
            for t in thresholds:
                ...
                if len(y_left) < len(y_right):
                    G_new += c * (1 - len(y_left) / (len(y_right) + 1))
                else:
                    G_new += c * (1 - len(y_right) / (len(y_left) + 1))
                if G_new < G:
                    threshold = t
        ...
        max_frac = 0.
        if len(y_left) < len(y_right):
            max_frac = len(y_left) / (len(y_right) + 1)
        else:
            max_frac = len(y_right) / (len(y_left) + 1)

        return feature_index, threshold, G, max_frac


    def get_lower_bound_on_c(self, X_subset, y_subset, parents_path, depth, min_frac, min_c, max_c, search_board=False):
        while (max_c - min_c > 5):
            cur_c = (min_c + max_c) / 2
            _, _, _, frac = self.choose_best_split(X_subset, y_subset, parents_path, depth, cur_c, search_board=False)
            if frac < min_frac:
                min_c = cur_c
            else:
                max_c = cur_c
        ...
        _, _, _, frac = self.choose_best_split(X_subset, y_subset, parents_path, depth, cur_c, search_board=False)
        if frac < min_frac and cur_c == min_c:
            return max_c 
        elif frac < min_frac and cur_c == max_c:
            return min_c
        return max_c

    def get_init_c(self, X_subset, y_subset, parents_path, depth, min_frac, search_board=False):
        max_c = min_c = self.c
        _, _, _, frac = self.choose_best_split(X_subset, y_subset, parents_path, depth, max_c, search_board=False)
        while frac < min_frac and max_c < 1e15:
            max_c *= 3
            _, _, _, frac = self.choose_best_split(X_subset, y_subset, parents_path, depth, max_c, search_board=False)
        while frac > min_frac and min_c > 1e-2:
            min_c /= 3
            _, _, _, frac = self.choose_best_split(X_subset, y_subset, parents_path, depth, min_c, search_board=False)
        return min_c, max_c
    ...
    def set_leaf(self, leaf_id, y_subset):
        ...
        self.impurity[leaf_id] = np.var(y_subset)
        self.feature[leaf_id] = -1
        self.threshold[leaf_id] = -1
        self.leaves_nums.append(leaf_id)
        if len(y_subset) > 0:
            if self.classification:
    ...
    def make_tree(self, X_subset, y_subset, parents_path=[]):
        ...
        """
        feature_index, threshold, impurity, _ = self.choose_best_split(X_subset, y_subset, parents_path, self.depth + 1)
        ...
        if self.depth < self.max_depth and X_subset.shape[0] >= self.min_samples_split:
            ...
            if min(len(y_left), len(y_right)) > 0:
                ...
                if self.classification:
                    self.probas[new_node.node_id] = [1 - np.mean(y_subset), np.mean(y_subset)]
    ...
    def recursive_predict(self, node_id, X, indices, y_predicted, probas=False):
        if self.children_left[node_id] == -1:
            ...
            (X_left, left_indices), (X_right, right_indices) = self.make_split(self.feature[node_id], self.threshold[node_id], X, indices)
            self.recursive_predict(self.children_left[node_id], X_left, left_indices, y_predicted, probas)
            self.recursive_predict(self.children_right[node_id], X_right, right_indices, y_predicted, probas)
